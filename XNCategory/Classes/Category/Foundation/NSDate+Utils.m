...
- (NSString *)stringYearMonthDayCompareToday
{
    ...
    //XNLog(@"dateStr----> chaDay %d",chaDay);
    if (chaDay == 0) {
        str = @"今天";
        //str = @"Today";
//        str = NSLocalizedString(@"ChatDateToday", nil);
    }else if (chaDay == 1){
        str = @"明天";
        //str = @"Tomorrow";
//        str = NSLocalizedString(@"ChatSendBtnTomorrow", nil);
    }else if (chaDay == -1){
        str = @"昨天";
        //str = @"Yesterday";
//        str = NSLocalizedString(@"ChatSendBtnYesterday", nil);
    }else if(chaDay == -2){
        str = @"前天";
    }else{
...
+ (NSString *)dateNowTime{
    return [self dateNowWithFormat:@"HH:mm"];
}
